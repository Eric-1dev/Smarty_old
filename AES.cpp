...
byte AES::cbc_encrypt (byte * plain, byte * cipher, int n_block, byte ivl [N_BLOCK])
{
  copy_n_bytes (iv, ivl, 16) ;
  while (n_block--)
    {
      xor_block (iv, plain) ;
...
byte AES::cbc_decrypt (byte * cipher, byte * plain, int n_block, byte ivl [N_BLOCK])
{   
  copy_n_bytes (iv, ivl, 16) ;
  while (n_block--)
    {
...
uint8_t AES::_getrnd() {
	...
	uint8_t really_random = rand() %256;
    return really_random;
}

/******************************************************************************/

void AES::gen_iv(byte  *iv) {
